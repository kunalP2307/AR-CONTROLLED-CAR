...
while Running:
    ...
    img = cv2.flip(img, 1)
    cv2.putText(img=img, text=str(orientation), org=(
        10, 20), fontFace=cv2.FONT_HERSHEY_TRIPLEX, fontScale=1, color=(0, 255, 0), thickness=3)
    ...
    if flag:
        ...
        wheel1 = pygame.transform.rotate(
            wheel1, math.degrees(math.atan(slope)))
